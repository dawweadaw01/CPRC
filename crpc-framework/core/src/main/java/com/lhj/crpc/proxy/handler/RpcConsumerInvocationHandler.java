...
import com.lhj.crpc.exceptions.NetworkException;
    ...
    public Object invoke(Object proxy, Method method, Object[] args) {
        ...
        while (true) {
            ...
            CrpcRequest crpcRequest = CrpcRequest.builder()
                    ...
                    .compressType(CompressorFactory.getCompressor(CrpcBootstrap.getInstance().getConfiguration().getCompressType()).getCode())
                    .requestType(RequestType.REQUEST.getId())
                    .serializeType(SerializerFactory.getSerializer(CrpcBootstrap.getInstance().getConfiguration().getSerializeType()).getCode())
                    .timeStamp(System.currentTimeMillis())
                    .requestPayload(requestPayload)
                    .build();

            // 2、将请求存入本地线程，需要在合适的时候remove
            CrpcBootstrap.REQUEST_THREAD_LOCAL.set(crpcRequest);

            // 3、发现服务，从注册中心拉取服务列表，并通过客户端负载均衡寻找一个可用的服务
            // 传入服务的名字,返回ip+端口
            InetSocketAddress address = CrpcBootstrap.getInstance()
            ...
            try {
                // 如果断路器是打开的
                if (crpcRequest.getRequestType() != RequestType.HEART_BEAT.getId() && circuitBreaker.isBreak()) {
                    // 定期打开
...
//                }
                ...
                // 我们可以想象得到，第一个出站程序一定是将 crpcRequest --> 二进制的报文
...
//                    }
                ...
                // 7、清理threadLocal
                CrpcBootstrap.REQUEST_THREAD_LOCAL.remove();
    ...
    private Channel getAvailableChannel(InetSocketAddress address) {
        ...
        // 1、尝试从缓存中获取
        Channel channel = CrpcBootstrap.CHANNEL_CACHE.get(address);
